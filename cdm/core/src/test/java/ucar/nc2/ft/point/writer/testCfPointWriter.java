...
public class testCfPointWriter {
  ...
  @Test
  public void testCoverageAsPoint1() throws IOException {
    String testFilePath1 = TestDir.cdmUnitTestDir + "ncss/climatology/PF5_SST_Climatology_Monthly_1985_2001.nc";
    CoverageCollection gds1 = CoverageDatasetFactory.open(testFilePath1).getCoverageCollections().get(0);
    LatLonPoint latlon1 = LatLonPoint.create(45.0, -20.0);
    double[] vals = Arrays.copyOfRange(expected, 0, 1);
    List<String> varNames = new ArrayList<>();
    varNames.add("sst");
    SubsetParams params = new SubsetParams();
    params.setVariables(varNames);
    params.setLatLonPoint(latlon1);
    // params.set("temporal", "all");
    // params.set("accept", "netcdf");
    writeCoverageAsPoint(varNames, params, 1);
  }

  @Test
  public void testWritePointCollection() throws IOException {
    List<String> varNames = new ArrayList<>();
    SubsetParams params = new SubsetParams();
    varNames.add("2D");
    params.setVariables(varNames);
    ...
    varNames.add("T1noZ");
    params.setVariables(varNames);
    writeCoverageAsPoint(varNames, params, 2);
    ...
    writeCoverageAsPoint(varNames, params, 4);


    // test multiple time axes and multiple levels
    varNames = new ArrayList<>();
    params = new SubsetParams();
    params.setLatLonPoint(latlon);
    varNames.add("full4");
    ...
    params.setVariables(varNames);
    // params.setVertCoord(alts[1]);
    writeCoverageAsPoint(varNames, params, 16);
  ...
  private void writeCoverageAsPoint(List<String> varNames, SubsetParams params, int countExpected) throws IOException {
    FeatureDatasetPoint fdp = new CoverageAsPoint(gds, varNames, params).asFeatureDatasetPoint();
    ...
    int count = CFPointWriter.writeFeatureCollection(fdp, fileOrg.getPath(), configOrg);
    ...
    int countNew = ucar.nc2.ft.point.writer2.CFPointWriter.writeFeatureCollection(fdp, fileNew.getPath(), configNew);
  ...
  private static void compare(String fileOrg, String fileNew) throws IOException {
    ...
    try (NetcdfFile org = NetcdfDatasets.openDataset(fileOrg); NetcdfFile copy = NetcdfDatasets.openDataset(fileNew)) {
      ...
      boolean ok = compare.compare(org, copy);
